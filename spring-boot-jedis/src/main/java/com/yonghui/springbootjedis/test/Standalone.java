 ...
 * 示例参考地址：https://www.cnblogs.com/c-xiaohai/p/8376364.html
 */
public class Standalone {
    ...
    public void testSortedSet() {
        ...
        map.put("key4",5.0);
        map.put("key5",0.2);
        System.out.println(jedis.zadd("zset", 0.3,"key1"));
    ...
    private static final String PRODUCT_KEY = "mykey";
    // 锁-实例
    ...
    public String getProductImgUrlById(String id) {
        // 获取缓存
        String product = jedis.get(PRODUCT_KEY + id);
        if (null == product) {
            ...
            try {
                if (lock.tryLock(3, TimeUnit.SECONDS)) {
                    try {
                        // 获取锁后再查一次，查到了直接返回结果
                        product = jedis.get(PRODUCT_KEY + id);
                        if (null == product) {
                            // ....
                        }
                        return product;
    ...
    /**
     * 解决缓存和数据库间数据一致性问题。双删且延时 + 设置数据过期时间
     * 参考地址： https://juejin.im/post/6844903878417186823#heading-0
     * 为什双删：https://blog.csdn.net/huizhi2533/article/details/107021249
     * @param key
     * @param data
     */
